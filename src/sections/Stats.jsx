...
function StatCard({ stat }) {
  ...
  return (
    <div
      ref={cardRef}
      className="glass rounded-3xl p-8 relative overflow-hidden transition-all duration-300 hover:border-primary/30 group"
    >
      ...
      <div className="relative z-10 flex flex-col justify-between h-full">
        <div>
          <span className="font-mono text-xs text-text-muted/60 uppercase tracking-widest block mb-4">
            {stat.label}
          </span>
          <div className="flex items-baseline font-jakarta font-black text-5xl sm:text-6xl tracking-tight mb-4">
            {stat.prefix && <span className="text-on-surface mr-1">{stat.prefix}</span>}
            <CountUp
              from={0}
              to={stat.value}
            ...
            />
            {stat.suffix && <span className="text-on-surface ml-1">{stat.suffix}</span>}
          </div>
        </div>
...
export default function Stats() {
  ...
  return (
    <section className="py-32 px-8 relative overflow-hidden bg-background">
      ...
      <div className="absolute top-1/3 right-1/4 -translate-y-1/2 w-[400px] h-[400px] bg-secondary/5 rounded-full blur-[100px] pointer-events-none" />
